...
// MSVC represents C enums as signed int (c_int), while GCC/Clang uses unsigned (c_uint).
// Provide a platform-aware alias so the rest of the file can use a single name.
#[cfg(all(windows, target_env = "msvc"))]
type _bindgen_c_enum = ::std::os::raw::c_int;
#[cfg(not(all(windows, target_env = "msvc")))]
type _bindgen_c_enum = ::std::os::raw::c_uint;
...
// wchar_t and max_align_t differ between Linux (c_int, 32 bytes) and Windows (u16, 8 bytes).
// Gate the Linux-specific definitions.
#[cfg(target_os = "linux")]
...
// ── Linux glibc types and C stdio functions ──
// These are pulled in by bindgen from Linux headers and are NOT part of the
// whisper.cpp / ggml API.  They cause compile-time size assertion failures on
// Windows (e.g. c_long is 4 bytes on Windows vs 8 on Linux).
// Gate the entire block so it only compiles on Linux.
#[cfg(target_os = "linux")]
mod _libc_bindings {
...
#[cfg(target_os = "linux")]
pub use _libc_bindings::*;

// On non-Linux platforms, provide an opaque FILE type for ggml_fopen() signature.
// The actual FILE struct is never accessed from Rust — only passed as an opaque pointer.
...
pub type ggml_op = _bindgen_c_enum;
...
pub type ggml_unary_op = _bindgen_c_enum;
...
pub type ggml_glu_op = _bindgen_c_enum;
...
pub type ggml_object_type = _bindgen_c_enum;
...
pub type ggml_log_level = _bindgen_c_enum;
...
pub type ggml_tensor_flag = _bindgen_c_enum;
...
pub type ggml_tri_type = _bindgen_c_enum;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
pub type ggml_op_pool = _bindgen_c_enum;
unsafe extern "C" {
    pub fn ggml_pool_1d(
        ctx: *mut ggml_context,
        a: *mut ggml_tensor,
...
pub type ggml_scale_mode = _bindgen_c_enum;
...
pub type ggml_scale_flag = _bindgen_c_enum;
unsafe extern "C" {
    pub fn ggml_upscale(
        ctx: *mut ggml_context,
        a: *mut ggml_tensor,
...
pub type ggml_sort_order = _bindgen_c_enum;
unsafe extern "C" {
    pub fn ggml_argsort(
        ctx: *mut ggml_context,
        a: *mut ggml_tensor,
...
pub const ggml_backend_buffer_usage_GGML_BACKEND_BUFFER_USAGE_COMPUTE: ggml_backend_buffer_usage =
    2;
pub type ggml_backend_buffer_usage = _bindgen_c_enum;
unsafe extern "C" {
...
pub type ggml_backend_dev_type = _bindgen_c_enum;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
pub type ggml_numa_strategy = _bindgen_c_enum;
unsafe extern "C" {
...
pub type whisper_alignment_heads_preset = _bindgen_c_enum;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
pub type whisper_gretype = _bindgen_c_enum;
#[repr(C)]
#[derive(Debug, Copy, Clone)]
...
pub type whisper_sampling_strategy = _bindgen_c_enum;
...
// __va_list_tag is x86_64 SysV ABI specific (Linux).
// On Windows, va_list is a simple pointer — these types are not needed.
#[cfg(target_os = "linux")]
